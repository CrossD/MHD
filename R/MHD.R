...
#'
#' Apply the metric halfspace depth algorithm (Dai and Lopez-Pintado, 2021+) to calculate the metric halfspace depth at a set of evaluation points with respect to a data cloud. Also calculate the deepest points both in- and out-of-sample.
#'
...
MHD <- function(mfd, data, anchors, XEval, theta0, 
  ...
  if (!depthOnly) {
    ...
    # origPar <- crossprod(basis, manifold::rieLog(mfd, theta0, orig))
    # origObjective <- -F(origPar)

    # Set the bounding box as the smallest one containing the 50% deepest samples
    V <- t(manifold::rieLog(mfd, theta0, t(data[depth >= median(depth), , drop=FALSE])))
